...
struct ProjectileStateModel: Equatable {
...
enum GameProtocol {
    ...
    static let projectileFlagSpawn: UInt8 = 0x01
    static let projectileFlagUpdate: UInt8 = 0x02
    static let projectileFlagDespawn: UInt8 = 0x04
    ...
    static func packProjectileState(_ projectile: ProjectileStateModel) -> Data {
        var data = Data()
        appendUInt16(UInt16(projectile.projectileId & 0xFFFF), to: &data)
        appendInt16(safeInt16(projectile.x), to: &data)
        appendInt16(safeInt16(projectile.y), to: &data)
        appendInt16(safeInt16(projectile.vx * velocityScale), to: &data)
        appendInt16(safeInt16(projectile.vy * velocityScale), to: &data)
        data.append(projectile.flags)
        return data
    }

    static func unpackProjectileState(_ payload: Data, ownerClientId: Int) -> ProjectileStateModel? {
        guard payload.count >= 11 else { return nil }
        let projectileId = Int(readUInt16(payload, at: 0))
        let x = Double(readInt16(payload, at: 2))
        let y = Double(readInt16(payload, at: 4))
        let vx = Double(readInt16(payload, at: 6)) / velocityScale
        let vy = Double(readInt16(payload, at: 8)) / velocityScale
        let flags = payload[10]
        return ProjectileStateModel(
